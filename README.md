...
[!WARNING] ⚠️ ユーザースクリプト導入に関する重要な警告

本ツールは「ブラウザ上で動作を書き換えるプログラム」です。以下のリスクを正しく理解し、自己責任で利用できる方のみ導入してください。

「野良スクリプト」の危険性: 悪意のあるスクリプトを導入すると、個人情報の窃取やアカウントの乗っ取りが行われる可能性があります。本スクリプトはオープンソース（公開）ですが、「誰が作ったか分からないプログラムを自分のブラウザで実行する」という行為のリスクを常に意識してください。

公式外のツール: 本ツールは「あにまん掲示板」公式とは一切無関係です。掲示板側の仕様変更により動作しなくなることや、予期せぬ不具合が発生する可能性があります。

中身の確認を推奨: 可能な限り、ご自身でソースコード（umakate.user.js）を確認し、納得した上でインストールしてください。

「よく分からないけど便利そうだから入れる」という安易な導入は推奨しません。
...
- **動作とデータ保存**: 本スクリプトはブラウザ上で動作するプログラムです。お気に入りや非表示、過去ログ等のデータは、すべてブラウザ内のデータベース（IndexedDB）に保存されます。外部サーバーへのデータ送信は一切行いません。
